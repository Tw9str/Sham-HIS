...
| Infrastructure | Local SQLite, transactional writes, audit history                                           | PostgreSQL migration, multi-hospital tenancy, distributed deployment, failover, backup/restore UI, encryption/key management, disaster recovery, observability, migrations framework |
...
- Use a single persistent Node process/local filesystem for this evaluation. SQLite is not configured for a serverless ephemeral filesystem or multi-region use.
...
- Known demo accounts are intentionally documented. Deployment scripts bind localhost. Do not tunnel or expose this demo to public networks.
